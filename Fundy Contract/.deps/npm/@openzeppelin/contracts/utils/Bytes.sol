...
library Bytes {
    ...
    function slice(bytes memory buffer, uint256 start, uint256 end) internal pure returns (bytes memory) {
        ...
        assembly ("memory-safe") {
            let srcPtr := add(add(buffer, 0x20), start)
    ...
    function replace(
        ...
        // replace
        assembly ("memory-safe") {
            let srcPtr := add(add(replacement, 0x20), offset)
            let destPtr := add(add(buffer, 0x20), pos)
    ...
    function concat(bytes[] memory buffers) internal pure returns (bytes memory) {
        ...
        for (uint256 i = 0; i < buffers.length; ++i) {
            ...
            assembly ("memory-safe") {
                ...
                let destPtr := add(result, offset)
                let len := mload(input)
                for { let j := 0 } lt(j, len) { j := add(j, 32) } {
                    mstore(add(destPtr, j), mload(add(srcPtr, j)))
                }
            }
            unchecked {
                offset += input.length;
